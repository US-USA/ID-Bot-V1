...
if [ ! -n "$BASH" ]; then
  ...
  bash -c '. <('"$(command -v curl >/dev/null && echo 'curl -Ls' || echo 'wget -qO-')"' https://raw.githubusercontent.com/US-USA/ID-Bot-V1/master/install.sh) '"$*"
  exit $?
fi
...
if [ -d "ID-Bot-V1/ID-Bot-V1" ]; then
  cd ID-Bot-V1 || { endspin "Failed to chdir"; exit 6; }
  DIR_CHANGED="yes"
fi
if [ -f ".setup_complete" ] || [ -d "ID-Bot-V1" -a ! x"" = x"$DYNO" ]; then
...
if echo "$OSTYPE" | grep -qE '^linux-gnu.*' && [ -f '/etc/debian_version' ]; then
  ...
  if [ ! "$(whoami)" = "root" ]; then
    ...
    if command -v sudo >/dev/null; then
      ...
      sudo "$BASH" -c '. <('"$(command -v curl >/dev/null && echo 'curl -Ls' || echo 'wget -qO-')"' https://raw.githubusercontent.com/US-USA/ID-Bot-V1/master/install.sh) '"$*"
      exit $?
    else
      PKGMGR="true"
    fi
  ...
  endspin "Unrecognised OS. Please follow https://ID-Bot-V1.gitlab.io/installing_advanced"
  exit 1
fi
...
${SUDO_CMD}rm -rf ID-Bot-V1
# shellcheck disable=SC2086
runout ${SUDO_CMD}git clone https://github.com/US-USA/ID-Bot-V1 || { errorout "Clone failed."; exit 3; }
cd ID-Bot-V1 || { endspin "Failed to chdir"; exit 7; }
# shellcheck disable=SC2086
...
${SUDO_CMD}"python$PYVER" -m ID-Bot-V1 "$@" || { echo "Python scripts failed"; exit 5; }
